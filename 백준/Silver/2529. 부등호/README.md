...
### 출력 
 ...
 1. 모든 숫자 조합을 전부 계산
 2. 해당 조합이 부등호를 만족시키는지 확인
 3. 만족시키는 경우 최대, 최솟값 설정

 python3로 할 경우 시간 초과, pypy3으로 할 경우 메모리 초과.
 최대/최소 값이 int(1e10) 이므로 최적화 필요.

 ### 정답 로직
 1. 숫자 조합을 만드는 도중에 부등호를 만족시키는지 확인
 2. 반복문 내부에서 현재까지 생성한 조합이 부등호를 만족시키는지 확인
 3. 만족시키는 경우에만 다음 재귀로 타고 들어감.

...
기존에는 상하좌우 좌표를 방문 표시하고 백트래킹을 진행했는데 시간초과가 심하게 걸렸다.

지나온 경로를 굳이 다 기록할 필요가 없다:  
 - 어차피 궁금한 정보는 알파벳을 사용했는지/하지 않았는지 여부이기 때문.

따라서 알파벳을 사용했는지 여부를 저장하는 배열을 두고 해당 알파벳이 사용중이지 않은 경우에만 재귀한다.

...
2차원 배열로 톱니 현황을 관리한다. 이 때 시간복잡도를 줄이기 위해 list 안에 deque를 넣는 방식으로 진행함.

각 index에 존재하는 deque는 해당 번호의 톱니를 의미한다. 인덱스 관리의 편의성을 위해 list의 첫 번째 항목은 빈 deque이다.

한 번의 iteration이 끝나고 dic에는 톱니의 회전 정보를 기록한다. 

`dic[key]=value` 형식에서 `key`: 톱니 번호, `value`: 회전 여부.

`value`의 값은 1: 시계 방향, 0: 변화 없음, -1: 반시계 방향으로 정했다.

1. 돌아갈 방향 체크하는 함수

  `체크` = 2번, 6번 인덱스가 같은지 다른지 여부를 판단하는 행위.


> n번 시계의 6번 인덱스 <-> n-1번 시계의 2번 인덱스  
> n번 시계의 2번 인덱스 <-> n+1번 시계의 6번 인덱스

  - 1번 톱니가 돌아가면 -> n+1만 체크하면 됨 
  - 4번 돌아가면 -> n-1만 체크하면 됨 
  - 2, 3번 ->  둘다 체크

  재귀 함수로 구현. 
  - solve -> ascend -> ascend -> ... 
  - solve -> descend -> descend -> ...

ascend: n+1 톱니바퀴 체크하는 함수  
descend: n-1 톱니바퀴 체크하는 함수
...
3. 정답 출력하는 함수   
모든 iteration이 끝나고 나서 기록해둔 톱니 현황을 기반으로 점수를 출력한다.

...
n = 100000 이므로 n^2 불가. n log n을 생각했을 때 이분탐색이라고 생각했다.

양수 = plus 배열, 음수 = minus 배열
음수 붙으면 음수 떼고 minus에 넣기
둘 다 정렬해놓고 plus 배열에 있는 숫자 하나마다 탐색.
가장 가까운 원소 인덱스 찾고 그 좌우로 차이 값의 절대값 비교
...
반례가 나온 이유: plus끼리 더한 경우에 최솟값이 나올 수 있음.
